  ...
  <div class='diagnosis-container'>
    ...
    <el-collapse-transition>
      <div class="diagnosis-info-content" v-show='showContent'>
        ...
        <p>外伤史：{{patientInfo.外伤史}}</p>
        <p>月经史：{{patientInfo.月经史}}</p>
        <p>家人病史：{{patientInfo.家人病史}}</p>
	  ...
	  <el-table :data='historyList' stripe border  :header-row-style='{background:"rgb(229,233,242)",color:"#475669"}' class='diagnosis-table-index1'>
...
export default {
  ...
  methods:{
    ...
    getOtherInfo(){
  	...
  	pageList(val){
  		console.log(val)
  		this.getInquiryList(val)
  	},

  	deleteExam(row){
  		console.log(row.idexId)
    ...
    getHistoryIndex(num){
      axios.get("api/inquiryResult/getInquiryResultList",{
        ...
        console.log("this.historyList",res)
      })
    }
  },
  created(){
